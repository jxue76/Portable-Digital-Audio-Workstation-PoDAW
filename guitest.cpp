...
int main(int, char**) {
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        if (current_state == SETTINGS) {
            ...
            char track_num[16];
            snprintf(track_num, sizeof(track_num), "%d", current_track + 1);
            ...
            /* Height: 320
                ...
                Height of each note: 280 / 24
                Width of each beat: 440 / (Numerator of time signature * 4) = 440 / 16
            */
           float note_height = 280/24;
           float note_width = 440/16;
           for (int i = 0; i < 24; i++) {
                for (int j = 0; j < 16; j++) {
                    if ((i % 2 == 0 && (j + 1) % 2 == 0) || ((i+1) % 2 == 0 && j % 2 == 0)) {
                        draw_list->AddRectFilled(ImVec2(p.x+40 + note_width*j, p.y + note_height*i),
                                                ImVec2(p.x+40 + note_width*(j+1), p.y + note_height*(i+1)),
